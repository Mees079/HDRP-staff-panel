const App = {
  ...
  ui: {
    ...
    showTab(tabName){
      document.querySelectorAll('.main-content').forEach(tab => tab.classList.remove('active'));
      const targetTab = document.getElementById(tabName+'Tab');
      if(targetTab) targetTab.classList.add('active');
      document.querySelectorAll('.nav-tab').forEach(btn=>btn.classList.remove('active'));
      const activeBtn = document.querySelector(`[data-tab="${tabName}"]`);
      if(activeBtn) activeBtn.classList.add('active');
      if(tabName==='users') App.ui.refreshUserTable();
    },
    updateUserDisplay(){
  ...
  auth: {
    ...
    addUser(email,password,role){
      ...
      if(App.data.users.find(u=>u.email===email)){App.ui.showMessage('addUserMessage','Gebruiker met dit email bestaat al');return false;}
      if(password.length<3){App.ui.showMessage('addUserMessage','Wachtwoord moet minimaal 3 karakters lang zijn'); return false;}
    ...
    changePassword(current,newPass,confirm){
      ...
      if(current!==App.data.currentUser.password){App.ui.showMessage('passwordMessage','Huidig wachtwoord is incorrect');return false;}
      if(newPass!==confirm){App.ui.showMessage('passwordMessage','Nieuwe wachtwoorden komen niet overeen');return false;}
      if(newPass.length<3){App.ui.showMessage('passwordMessage','Wachtwoord moet minimaal 3 karakters lang zijn');return false;}
  ...
  init(){
    ...
    if(addUserForm){
      const addBtn = addUserForm.querySelector('button[type="submit"]');
    ...
    if(passwordForm){
      const passBtn = passwordForm.querySelector('button[type="submit"]');
      if(passBtn) passBtn.addEventListener('click',function(e){
        e.preventDefault();
        const current=document.getElementById('currentPasswordInput').value;
        const newP=document.getElementById('newPasswordInput').value;
        const confirm=document.getElementById('confirmPasswordInput').value;
        App.auth.changePassword(current,newP,confirm);
      });
    }
