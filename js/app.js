...
const App = {
    ...
    storage: {
        save() {
            console.log('Data opgeslagen in geheugen:', JSON.stringify(App.data, null, 2));
        },
        load() {
            console.log('Data geladen uit geheugen');
        }
    },

    ui: {
        showMessage(elementId, message, type = 'error', duration = 5000) {
    ...
    init() {
        ...
        if (themeToggle) themeToggle.addEventListener('click', App.ui.toggleTheme);

        // User Menu
        const userButton = document.getElementById('userButton');
        if (userButton) userButton.addEventListener('click', App.ui.toggleUserMenu);

        const logoutButton = document.getElementById('logoutBtn');
        if (logoutButton) logoutButton.addEventListener('click', App.auth.logout);
